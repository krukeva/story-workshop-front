...
import { getOneEquipment } from "../../services/equipmentService"
...
function StarButton({ name, value, size, fontSize }) {
  return (
    <RoundButton
      name={name}
      value={value}
      color={value !== "true" ? colors.equipment : "none"}
      size={size}
      fontSize={fontSize}
    >
      <StarDiv state={value}>
...
export async function loader({ params }) {
  const equipment = await getOneEquipment(params.equipmentId)
  if (!equipment) {
    throw new Response("", {
      status: 404,
  ...
  return (
    <Wrapper>
      <FixedDiv top="15px" right="50px">
        <Form action="edit">
          <EditButton type="submit" color={colors.equipment} />
        </Form>
      </FixedDiv>
      ...
      <DataTemplate>
        <DataTemplate.Name>
          <StarredName>
            ...
            {typeof equipment.worldId === "string" ? (
              <span>
                <FontAwesomeIcon icon={faGlobe} />
              </span>
            ) : (
              <Favorite item={equipment} />
            )}
          </StarredName>
        </DataTemplate.Name>
